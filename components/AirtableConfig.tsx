    ...
    const handleConnect = async () => {
        ...
        try {
            ...
            if (success) {
                ...
                setStatus('error');
                setErrorMsg('No se pudo conectar. Compruebe el ID de la Base y el Nombre de la Tabla.');
            }
        } catch (e) {
            setStatus('error');
            setErrorMsg('Error de conexión. Compruebe su Token de Acceso Personal.');
        }
    };
    ...
    return (
        <div className="max-w-3xl mx-auto p-8 animate-fade-in">
            <div className="mb-8">
                ...
                <p className="text-slate-500">Sincronice sus siniestros y peritaciones directamente con una Base de Airtable.</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-yellow-100 p-8 relative overflow-hidden">
                ...
                {status === 'connected' ? (
                    <div className="text-center py-8 animate-fade-in-up">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-2">Conectado a Airtable</h3>
                        <p className="text-slate-500 mb-1">Base: <strong>{config.baseId}</strong></p>
                        <p className="text-slate-500 mb-8">Tabla: <strong>{config.tableName}</strong></p>
                    ...
                    <div className="space-y-6">
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 text-sm text-slate-600 mb-6">
                            <p className="font-bold mb-2">Requisitos previos:</p>
                            <ul className="list-disc pl-5 space-y-1">
                                <li>Cree una tabla en Airtable con estas columnas (Sensible a Mayúsculas):</li>
                                <li className="font-mono text-xs bg-slate-200 inline-block px-1 rounded">ID, Date, Status, Vehicle, Plate, Insured, Company</li>
                                <li>Genere un <strong>Personal Access Token</strong> con el permiso <code>data.records:write</code>.</li>
                            </ul>
                        </div>

                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">Token de Acceso Personal (PAT)</label>
                            <input
                                type="password"
                                name="apiKey"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">ID de la Base</label>
                                <input
                                    type="text"
                                    name="baseId"
                                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-yellow-400 outline-none text-sm font-mono"
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">Nombre de la Tabla</label>
                                <input
                                    type="text"
                                    name="tableName"
                        ...
                        {status === 'error' && (
                            <div className="bg-red-50 text-red-600 p-3 rounded-lg text-sm flex items-center gap-2">
                                ...
                                {errorMsg || 'Fallo de conexión. Compruebe las credenciales.'}
                            </div>
                        )}
