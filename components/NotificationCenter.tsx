            ...
            {isOpen && (
                <div className="absolute left-0 top-full mt-2 w-full lg:w-72 bg-white rounded-xl shadow-xl border border-slate-200 z-50 overflow-hidden animate-fade-in-up">
                    <div className="p-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                        <h3 className="text-xs font-bold text-slate-500 uppercase">Centro de Notificaciones</h3>
                        {notifications.length > 0 && (
                            <button onClick={handleClear} className="text-xs text-brand-600 hover:underline">Limpiar Todo</button>
                        )}
                    </div>
                            ...
                            notifications.map(note => (
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${note.type === 'success' ? 'bg-green-500' :
