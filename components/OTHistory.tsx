...
const OTHistory: React.FC = () => {
    ...
    const handleViewProfitability = async (valuationId: string, orderId: string) => {
        ...
        try {
            ...
            if (!request || !request.file_url) {
                alert(`No se encontró el documento de análisis para la peritación vinculada a esta reparación.\n\nPor favor, asegúrese de que el documento haya sido analizado en el chat de este siniestro primero.`);
                setAnalyzingId(null);
                return;
            }
            ...
            reader.onloadend = async () => {
                ...
                if (!analysisData) {
                    alert("Fallo en el análisis. No se pudieron extraer datos del documento.");
                    setAnalyzingId(null);
                    return;
                }
            ...
            console.error(e);
            alert("Error al generar el informe de rentabilidad.");
            setAnalyzingId(null);
        }
    };
    ...
    const handleDelete = async (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
        if (window.confirm("¿Está seguro de que desea eliminar permanentemente esta Orden de Trabajo?")) {
            const result = await deleteWorkOrder(id);
            if (result.success) {
                setHistory(prev => prev.filter(job => job.id !== id));
            } else {
                alert(`Fallo al eliminar: ${result.error}`);
            }
        }
    };
    ...
    const getStatusBadge = (status: string) => {
        ...
        if (s === 'finished' || s === 'invoiced' || s === 'closed') {
            color = 'bg-green-100 text-green-700 border-green-200';
            label = s === 'finished' ? 'Finalizado' : s === 'invoiced' ? 'Facturado' : 'Cerrado';
        }
        else if (s === 'reception') {
            color = 'bg-blue-100 text-blue-700 border-blue-200';
            label = 'Recepción';
        }
        else if (['bodywork', 'paint', 'disassembly'].includes(s)) {
            color = 'bg-orange-100 text-orange-700 border-orange-200';
            label = s === 'bodywork' ? 'Chapa/Mec' : s === 'paint' ? 'Pintura' : 'Desmontaje';
        }
        else if (s === 'admin_close') {
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                        ...
                        Historial de Reparaciones y Facturación
                    </h1>
                    <p className="text-slate-500 text-sm font-medium">Seguimiento de costes en tiempo real desde los registros de producción del taller.</p>
                </div>
                ...
                <div className="overflow-x-auto flex-1">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {isLoading && (
                                <tr><td colSpan={7} className="p-16 text-center text-slate-400 font-bold uppercase tracking-widest text-[10px]">Sincronizando historial...</td></tr>
                            )}
                            {!isLoading && filteredHistory.length === 0 && (
                                <tr><td colSpan={7} className="p-20 text-center text-slate-400 font-bold italic">No se encontraron registros.</td></tr>
                            )}
                                ...
                                return (
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors cursor-pointer group" onClick={() => navigate(`/expediente/${item.id}`)}>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="font-bold text-slate-800 text-sm">{item.vehicle || 'Desconocido'}</div>
                                            <div className="text-[10px] text-slate-400 font-mono bg-slate-100/50 inline-block px-1.5 py-0.5 rounded border border-slate-200 mt-1 uppercase">{item.plate || 'Sin Matrícula'}</div>
                                        </td>
