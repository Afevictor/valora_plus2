...
const ChatBot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: '1', role: 'model', text: '¡Hola! Soy el asistente virtual de Valora Plus. ¿Cómo puedo ayudarle con su taller hoy?' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'model', text: 'Lo siento, tuve problemas para conectar. Por favor, inténtelo de nuevo.' }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const handleClear = () => {
    setMessages([
      { id: Date.now().toString(), role: 'model', text: '¡Hola! Soy el asistente virtual de Valora Plus. ¿Cómo puedo ayudarle con su taller hoy?' }
    ]);
        ...
        <div className="fixed bottom-24 right-6 w-96 h-[500px] bg-white rounded-2xl shadow-2xl flex flex-col z-40 border border-slate-200 overflow-hidden animate-fade-in-up">
          ...
          <div className="bg-brand-600 p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-white font-semibold">Asistente Valora</h3>
                <p className="text-brand-100 text-xs">Potenciado por Gemini</p>
              </div>
            </div>
            <button onClick={handleClear} className="text-white/70 hover:text-white transition-colors" title="Limpiar Chat">
