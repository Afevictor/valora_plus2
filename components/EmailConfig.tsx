...
const EmailConfig: React.FC = () => {
    ...
    const handleDisconnect = () => {
        if (window.confirm('¿Eliminar la configuración de email?')) {
    ...
    return (
        <div className="max-w-3xl mx-auto p-8 animate-fade-in">
            <div className="mb-8">
                ...
                <p className="text-slate-500">Configure el correo electrónico de su taller para comunicarse con expertos directamente desde el chat.</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-blue-100 p-8 relative overflow-hidden">
                ...
                {status === 'connected' ? (
                    <div className="text-center py-8 animate-fade-in-up">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-2">Email Conectado</h3>
                        <p className="text-slate-500 mb-1">Cuenta: <strong>{config.email}</strong></p>
                        <p className="text-xs text-slate-400 mb-8">Servidor: {config.host}:{config.port}</p>

                        <div className="bg-blue-50 text-blue-800 text-xs p-3 rounded mb-6 max-w-sm mx-auto">
                            <strong>Modo Simulación Activo:</strong> Los correos electrónicos se simularán mediante notificaciones Toast y respuestas automáticas. No se enviarán correos reales en este entorno de demo.
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold text-slate-700 mb-2">Dirección de Email del Taller</label>
                                <input
                                    type="email"
                                    name="email"
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold text-slate-700 mb-2">Nombre del Remitente</label>
                                <input
                                    type="text"
                                    name="senderName"
                                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">Host SMTP</label>
                                <input
                                    type="text"
                                    name="host"
                                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">Puerto SMTP</label>
                                <input
                                    type="text"
                                    name="port"
                                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold text-slate-700 mb-2">Contraseña de Aplicación / Clave API</label>
                                <input
                                    type="password"
                                    name="password"
                                    ...
                                    placeholder="••••••••••••"
                                />
                                <p className="text-xs text-slate-400 mt-1">Recomendamos usar una Contraseña de Aplicación por seguridad.</p>
                            </div>
                        </div>
                        ...
                        {status === 'error' && (
                            <div className="bg-red-50 text-red-600 p-3 rounded-lg text-sm flex items-center gap-2">
                                ...
                                Fallo de conexión. Por favor, compruebe sus ajustes.
                            </div>
                        )}
                        ...
                        <div className="pt-4 flex gap-3 justify-end border-t border-slate-100">
                            ...
                            >
                                {status === 'checking' ? 'Conectando...' : 'Guardar y Conectar'}
                            </button>
                        </div>
                    </div>
                )}
